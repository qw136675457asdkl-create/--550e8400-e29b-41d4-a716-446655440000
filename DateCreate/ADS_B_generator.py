...
def encode_bcd_byte(value: int) -> int:
    ...
    if normalized < 0 or normalized > 99:
        raise ValueError(f"BCD byte value out of range: {value}")
    return ((normalized // 10) << 4) | (normalized % 10)


def build_timestamp_prefix_bytes(start_datetime: datetime, elapsed_seconds: float) -> bytes:
    current_time = start_datetime + timedelta(seconds=float(elapsed_seconds))
    millisecond_text = f"{current_time.microsecond // 1000:04d}"
    return bytes(
        [
            encode_bcd_byte(current_time.hour),
            encode_bcd_byte(current_time.minute),
            encode_bcd_byte(current_time.second),
            encode_bcd_byte(int(millisecond_text[:2])),
            encode_bcd_byte(int(millisecond_text[2:])),
        ]
    )
...
def generate_wingman_message_dataframe(
    ...
    for elapsed_seconds, lon_value, lat_value, alt_value in zip(sample_times, sample_lon, sample_lat, sample_z):
        ...
        timestamp_prefix = build_timestamp_prefix_bytes(start_datetime, float(elapsed_seconds))
        full_packet_bytes = timestamp_prefix + packet_bytes
        row = {
            "_elapsed_seconds": float(elapsed_seconds),
            "_full_datetime": format_full_datetime(start_datetime, elapsed_seconds),
            "时间戳前缀": format_hex_bytes(timestamp_prefix),
            ...
            "十六进制报文": full_packet_bytes.hex().upper(),
        }
        rows.append(row)
